...
upsample.fun= function(data){
  data$target = as.factor(data$target)
  data = upSample(data, data$target)
  data = data[,-ncol(data)]
  return(data) 
}
...
system.time(for(iter in 1:n){
  ...
  sam = sqrt.fun(upsample.fun(train[nset, ]))
...
m_rf_parm = expand.grid(ntree=c(300), mtry = c(1), nodesize = c(1))
...
or_rf_parm = expand.grid(ntree=c(400), mtry = c(1), nodesize = c(2))
...
or_xgb_parm = expand.grid(eta = c(0.05), max_depth = c(3), nrounds = c(200))
...
colnames(f.rst.tab) = c('m_rf', 'or_rf', 'or_xgb')
