...
smote.fun = function(data){
  data$target = as.factor(data$target)
  t = table(data$target)[order(table(data$target))]
  for(iter in 1:length(unique(data$target))){
    if(t[iter]>=max(t)/2) break
    data = SMOTE(data[,-ncol(data)], data[,ncol(data)], K = 3, dup_size = ceiling((max(t)-t[iter])/t[iter]))
    data = data$data
  }
  colnames(data)[ncol(data)] = 'target'
  data$target = as.factor(data$target)
  data$강수여부 = round(data$강수여부)
  return(data)
}
...
system.time(for(iter in 1:n){
  ...
  sam = sqrt.fun(smote.fun(train[nset, ]))
...
m_lgb_grid = function(dat, r, parm){
  lgb.auc.mat = matrix(NA, r, nrow(parm))
  set = sample(1:nrow(dat), 0.5*nrow(dat))
  train = dat[set,]
  test = dat[-set,]
  for(iter in 1:r){
    ...
    for(iiter in 1:nrow(parm)){
      params = list(objective = "multiclass", metric="multi_logloss", num_class = length(unique(dat$target)),
                    learning_rate = parm[iiter,"learning_rate"] ,num_leaves = parm[iiter,"num_leaves"] ,max_depth = parm[iiter,"max_depth"])
      g_model = lgb.train(data = lgb_train, nrounds = parm[iiter,"nrounds"], params = params)
      g_pred = max.col(predict(g_model, as.matrix(val[, -ncol(dat)]), reshape=T))-1 #numeric
      lgb.auc.mat[iter,iiter] = multiclass.roc(val$target, g_pred)$auc[1]
    }
  }
  plot(apply(lgb.auc.mat,2,median), type='b', col='lightseagreen')
  print(cbind(apply(lgb.auc.mat,2,median),parm)[order(cbind(apply(lgb.auc.mat,2,median),parm)[,1], decreasing = TRUE),])
  opt = parm[which.max(apply(lgb.auc.mat,2,median)),]
  return(opt)
}

m_lgb_parm = expand.grid(learning_rate = c(0.05), num_leaves = c(25), max_depth = c(7), nrounds = c(300))

system.time({m_lgb_opt = m_lgb_grid(dat, r, m_lgb_parm)})
m_lgb_opt
# 1개 50초
...
#-----------------------------------------lightgbm_one_vs_rest
or_lgb_grid = function(dat, r, parm){
  lgb.auc.mat = matrix(NA, r, nrow(parm))
  set = sample(1:nrow(dat), 0.5*nrow(dat))
  train = dat[set,]
  test = test = dat[-set,]
  ...
  for(iter in 1:r){
    ...
    for(iiter in 1:nrow(parm)){
      ...
      for(iiiter in 1:length(unique(dat$target))){
        ...
        lgb_train = lgb.Dataset(data = as.matrix(orsam[, -ncol(dat)]), label = orsam$target)
        g_model = lgb.train(data = lgb_train, nrounds = parm[iiter,"nrounds"], params = params)
        prob_pred[,iiiter] = predict(g_model, as.matrix(val[,-ncol(dat)]))
      }
      g_pred = max.col(prob_pred)-1
      lgb.auc.mat[iter,iiter] = multiclass.roc(val$target, g_pred)$auc[1]
    }
...
or_lgb_parm = expand.grid(learning_rate = c(0.1), num_leaves = c(20), max_depth = c(7), nrounds = c(200))

system.time({or_lgb_opt = or_lgb_grid(dat, r, or_lgb_parm)})
or_lgb_opt
# 1개 40초

f.rst.tab = matrix(NA, 3, 3)
rownames(f.rst.tab) = c("acc", "kpa", "auc")
colnames(f.rst.tab) = c('m_lgb', 'or_rf', 'or_lgb')
...
system.time(for(iter in 1:n){
  ...
  lgb_train = lgb.Dataset(data = as.matrix(sam[,-ncol(dat)]), label = sam$target)
  # lightGBM 모델 학습
  lgb_params = list(objective = "multiclass", metric="multi_logloss", num_class = length(unique(dat$target)),
                    learning_rate = m_lgb_opt[1,"learning_rate"] ,num_leaves = m_lgb_opt[1,"num_leaves"] ,max_depth = m_lgb_opt[1,"max_depth"])
  lgb_model = lgb.train(data = lgb_train, nrounds = m_lgb_opt[1,"nrounds"], params = lgb_params)
  # 예측 수행
  lgb_pred = max.col(predict(lgb_model,  as.matrix(test[, -ncol(dat)]), reshape=T))-1 #numeric
  # 예측 결과 평가
  acc.mat[iter,1] = confusionMatrix(as.factor(lgb_pred), test$target)$overall[1]
  kpa.mat[iter,1] = confusionMatrix(as.factor(lgb_pred), test$target)$overall[2]
  auc.mat[iter,1] = multiclass.roc(test$target, lgb_pred)$auc[1]
  ...
  rf_pred = max.col(prob_pred)-1
  # 예측 결과 평가
  acc.mat[iter,2] = confusionMatrix(as.factor(rf_pred), test$target)$overall[1]
  kpa.mat[iter,2] = confusionMatrix(as.factor(rf_pred), test$target)$overall[2]
  auc.mat[iter,2] = multiclass.roc(test$target, rf_pred)$auc[1]
  ...
  for(iiter in 1:length(unique(dat$target))){
    ...
    # lightGBM 모델 학습
    lgb_train = lgb.Dataset(data = as.matrix(orsam[, -ncol(dat)]), label = orsam$target)
    lgb_params = list(objective = "binary", metric="binary_logloss",
                      learning_rate = or_lgb_opt[1,"learning_rate"] ,num_leaves = or_lgb_opt[1,"num_leaves"] ,max_depth = or_lgb_opt[1,"max_depth"])
    lgb_model = lgb.train(data = lgb_train, nrounds = or_lgb_opt[1,"nrounds"], params = lgb_params)
    # 예측 수행
    prob_pred[,iiter] =  predict(lgb_model, as.matrix(test[,-ncol(dat)]))
  }
  # 가장 높은 확률 값을 가진 클래스 선택
  lgb_pred = max.col(prob_pred)-1
  # 예측 결과 평가
  acc.mat[iter,3] = confusionMatrix(as.factor(lgb_pred), test$target)$overall[1]
  kpa.mat[iter,3] = confusionMatrix(as.factor(lgb_pred), test$target)$overall[2]
  auc.mat[iter,3] = multiclass.roc(test$target, lgb_pred)$auc[1]
