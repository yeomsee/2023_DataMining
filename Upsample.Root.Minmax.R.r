...
system.time(for(iter in 1:n){
  ...
  osam = sqrt.fun(upsample.fun(train[nset,]))
...
m_rf_parm = expand.grid(ntree=c(500), mtry = c(2), nodesize = c(2))
...
m_xgb_parm = expand.grid(eta = c(0.1), max_depth = c(5), nrounds = c(200))
...
colnames(f.rst.tab) = c('m_rf', 'm_xgb', 'or_rf')
