...
system.time(for(iter in 1:n){
  ...
  osam = sqrt.fun(smote.fun(train[nset,]))
...
m_lgb_parm = expand.grid(learning_rate = c(0.1), num_leaves = c(26), max_depth = c(10), nrounds = c(200))
...
or_rf_parm = expand.grid(ntree=c(300), mtry = c(3), nodesize = c(1))
...
or_xgb_grid = function(dat, r, parm){
...
or_xgb_parm = expand.grid(eta = c(0.1), max_depth = c(6), nrounds = c(400))

system.time({or_xgb_opt = or_xgb_grid(dat, r, or_xgb_parm)})
or_xgb_opt
# 1개 105초

f.rst.tab = matrix(NA, 3, 3)
rownames(f.rst.tab) = c("acc", "kpa", "auc")
colnames(f.rst.tab) = c('m_lgb', 'or_rf', 'or_xgb')
...
system.time(for(iter in 1:n){
  ...
  for(iiter in 1:length(unique(dat$target))){
    ...
    xgb_train = xgb.DMatrix(data = as.matrix(orsam[,-ncol(dat)]), label = orsam$target)
    xgb_params = list(objective = "binary:logistic",
                      eta = or_xgb_opt[1,"eta"], max_depth = or_xgb_opt[1,"max_depth"])
    xgb_model = xgb.train(data = xgb_train, nrounds = or_xgb_opt[1,"nrounds"], params = xgb_params)
    # 예측 수행
    prob_pred[,iiter] =  predict(xgb_model, as.matrix(test[,-ncol(dat)]))
  }
  # 가장 높은 확률 값을 가진 클래스 선택
  xgb_pred = max.col(prob_pred)-1
  # 예측 결과 평가
  acc.mat[iter,3] = confusionMatrix(as.factor(xgb_pred), test$target)$overall[1]
  kpa.mat[iter,3] = confusionMatrix(as.factor(xgb_pred), test$target)$overall[2]
  auc.mat[iter,3] = multiclass.roc(test$target, xgb_pred)$auc[1]
