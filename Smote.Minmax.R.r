...
m_rf_parm = expand.grid(ntree=c(400), mtry = c(2), nodesize = c(2))
...
m_lgb_parm = expand.grid(learning_rate = c(0.05), num_leaves = c(13), max_depth = c(8), nrounds = c(400))
...
or_rf_parm = expand.grid(ntree=c(300), mtry = c(2), nodesize = c(1))
...
colnames(f.rst.tab) = c('m_rf', 'm_lgb', 'or_rf')
...
system.time(for(iter in 1:n){
  ...
  # 예측 결과 평가
  acc.mat[iter,2] = confusionMatrix(as.factor(lgb_pred), test$target)$overall[1]
  kpa.mat[iter,2] = confusionMatrix(as.factor(lgb_pred), test$target)$overall[2]
  auc.mat[iter,2] = multiclass.roc(test$target, lgb_pred)$auc[1]
